  ...
  try {
    ...
    if (checkAvailable.length !== 0) {
      const error = new Error(
        "Can't add new schedule due to overlapping time."
      );
      error.statusCode = 400;
      throw error;
    }
    ...
    const newMeetingWithCorrectTimezone = {
      ...newMeeting.toJSON(),
      datetime_start: formatToTimeZone(
        newMeeting.datetime_start,
        "YYYY-MM-DD HH:mm:ss",
        { timeZone: "Asia/Jakarta" }
      ),
      datetime_end: formatToTimeZone(
        newMeeting.datetime_end,
        "YYYY-MM-DD HH:mm:ss",
        { timeZone: "Asia/Jakarta" }
      ),
    };
...
const getMeetingDetails = async (req, res, next) => {
  try {
    ...
    const meetingDetailsWithCorrectTimezone = meeting_details.map(
      (meeting_detail) => ({
        ...meeting_detail.toJSON(),
        datetime_start: formatToTimeZone(
          meeting_detail.datetime_start,
          "YYYY-MM-DD HH:mm:ss",
          { timeZone: "Asia/Jakarta" }
        ),
        datetime_end: formatToTimeZone(
          meeting_detail.datetime_end,
          "YYYY-MM-DD HH:mm:ss",
          { timeZone: "Asia/Jakarta" }
        ),
      })
...
const bookSpecificMeeting = async (req, res, next) => {
  try {
    ...
    if (specificMeeting.studentId != null) {
      const error = new Error(
        `Can't book schedule with ID ${scheduleId}! The requested time slot is already booked by another meeting.`
      );
      error.statusCode = 400;
      throw error;
    }
    ...
    if (checkAvailable.length == 0) {
      ...
      const error = new Error(
        "Cannot book the schedule because you already have another meeting at the selected time."
      );
      error.statusCode = 400;
      throw error;
    }
    const specificMeetingsWithCorrectTimezone = {
      ...specificMeeting.toJSON(),
      datetime_start: formatToTimeZone(
        specificMeeting.datetime_start,
        "YYYY-MM-DD HH:mm:ss",
        { timeZone: "Asia/Jakarta" }
      ),
      datetime_end: formatToTimeZone(
        specificMeeting.datetime_end,
        "YYYY-MM-DD HH:mm:ss",
        { timeZone: "Asia/Jakarta" }
      ),
    };
  ...
  try {
    ...
    if (currentUser.role == "Student") {
      ...
      bookedMeeting = bookedMeeting.map((bookedMeeting) => ({
        ...
        datetime_start: formatToTimeZone(
          bookedMeeting.datetime_start,
          "YYYY-MM-DD HH:mm:ss",
          { timeZone: "Asia/Jakarta" }
        ),
        datetime_end: formatToTimeZone(
          bookedMeeting.datetime_end,
          "YYYY-MM-DD HH:mm:ss",
          { timeZone: "Asia/Jakarta" }
        ),
      }));
